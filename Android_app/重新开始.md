...
class attribution():
    ...
    #当数值的上限下限被修改
    def limit(self,owner,lim:list,world_state={}):
        assert self.valuable == True
        #同样,汇报上下限变化
        world_state['subject'] = owner
        world_state['attribute'] = self
        world_state['lim'] = lim
    ...
    def attach(self,owner=None):
        #检测owner是否没有输入, 如果没有输入就看这个属性本身是否已经预先定义了owner
        if owner == None:
            owner = self.owner
            assert owner != None
        #这个和action的注册相似,
