...
经典respose函数:
```python
def response(owner,repeat:int,event,world_state:dict):
    #所有的对象应当是全局变量,因为可以这样判定event是否是自己想要的event

    if owner in world_state['attackee']:
        pass
    elif owner in world_state['attacker']:
        pass
...
buff1 = attribution(id=0,name='血量',description={'我是个':'字典'},valuable=True,reg_func_list=reg_func_list,event_list_on_value_change=[event_id4],event_list_on_limit_change=[event_id5])
hp = attribution(id=0,name='亡语',description={'我是个':'字典'},valuable=False,reg_func_list=reg_func_list,event_list_on_value_change=[event_id4],event_list_on_limit_change=[event_id5])
...
class event():
    ...
    def run(self,world_state:dictionary):
        for i in self.func_list:
            ...
            ops,world_state = i['func'](owner=i['owner'],repeat=i['repeat'],event=self,world_state) #将owner放在最前面,这样可以直接满足一些函数的self
...
class unit():
    ...
    def act(self,action,world_state):
        action.do(self,world_state)
