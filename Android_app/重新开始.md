...
class event():
    ...
    def run(self,world_state:dictionary):
        for i in self.func_list:
            ...
            ops = i['func'](event=self,repeat=i['repeat'],owner=i['owner'],world_state)
...
class attribution():
    ...
    def __init__(self,*args,**kwargs):
        ...
        if self.valuable == True:
            ...
            self.limit = kwargs['limit'] = None
            self.limit = kwargs['value'] = None

        #因为这个东西是大类,所以没法定义唯一的owner
        #这个和下面的action类中的一样
        #这个仅仅表示向event注册哪些基于时间点的函数
        self.reg_func_dict = kwargs['reg_func_dict']

    def attach(self):
        #这是无效函数,因为需要一个属性被多个内容拥有
...
class action():
    ...
    def activate(self,owner,world_state=None):
        self.setup(owner)
